...
\begin{figure}[!ht]
	...
	\caption{Streaming Layer}
	\label{fig:streamlayer}
\end{figure}


En la figura \ref{fig:streamlayer} 
...
En esta primera sección, antes de empezar con la definición de la capa de \textit{streaming}, vamos a recapitular los requisitos que deberá cumplir el sistema encargado de clasificar las llamadas recibidas del \textit{call center} en tiempo de real. 
...
El núcleo de nuestra capa de \textit{Streaming} estará compuesto por microservicios desarrollados mediante 
 \textit{Kafka Streams}. \textit{Kafka Streams} \cite{kstreams} es una librería cliente que nos permite desarrollar aplicaciones y microservicios  cuya entrada y salida sea un bus \textit{Kafka}. 
 
 \textit{Kafka Streams} nos permite desarrollar aplicaciones en \textit{Java} (el lenguaje que usaremos) o \textit{Scala} de una manera sencilla e intuitiva, además, a diferencia de otros \textit{frameworks} de procesamiento en tiempo real, como puede ser \textit{Spark Streaming}, no tiene necesidad de disponer de un clúster aparte (usa el mismo bus \textit{Kafka}) lo que simplifica bastante nuestra arquitectura. Además \textit{Kafka Streams} nos permitirá \textbf{garantizar la escalabilidad y la tolerancia a fallos} de nuestros despliegues. 
 
 \textit{Kafka Streams} posee dos \textit{APIs} diferentes \textit{Processor API} y  \textit{Streams DSL}.  \textit{Processor API} nos proporciona una capacidad a muy bajo nivel para definir la lógica de nuestro proceso \textit{streaming}; en cambio \textit{DSL}, que esta construido sobre  \textit{Processor API}, nos permite de una manera sencilla, con un lenguaje declarativo, realizar la mayoría de operaciones posibles en un proceso de \textit{streaming}. Es por ello que en nuestros servicios utilizaremos  \textit{\textbf{Streams DSL}}.
 
Una característica interesante de \textit{Streams DSL} es la existencia de las denominadas \textit{KTables} que nos permiten usar el Bus como si de una BBDD NoSQL Clave-Valor se tratara. Las \textit{Ktables} utilizan un \textit{topic} de \textit{Kafka}, pero al acceder a ellas solo accedemos al último registro disponible para cada clave. Esta característica suele usarse sobre \textit{topics} compactos sin límite de retención, que son aquellos que periódicamente  borran las versiones antiguas de cada clave y retienen, de forma ilimitada, las versiones más recientes. Esta posibilidad nos permitirá cargar en el bus las tablas de \textit{lookup} necesarias para enriquecer registros en nuestro procesamiento y eliminará las dependencias de nuestro flujo con BBDD externas. 
...
\textit{Tensorflow Serving} \cite{tfserving} está pensado para llevar a producción los modelos que hemos creado con \textit{Tensor Flow}, pudiéndose adaptar también para otros modelos. \textit{Tensorflow Serving} nos permite desplegar nuestros modelos y algoritmos manteniendo intacta la \textit{API} de consulta, esto lo hace ideal para entornos en los que queremos ir actualizando las versiones de nuestros modelos sin modificar el resto del sistema.

Los modelos desplegados con \textit{Tensorflow Serving} pueden consultarse a través de un \textit{API} en \textit{C++} o mediante un \textit{\textbf{API REST}} que es la opción escogida por nosotros debido a que, por la simplicidad del protocola, es la que más encaja con nuestra arquitectura de microservicios. 
...
un topic compacto sin límite de retención. Como salida escribirá en un \textit{topic}  llamado \textit{SEQUENCES.CALLS} un evento cuya clave será el código de la llamada y cuyo cuerpo un objeto \textit{json} con los siguientes campos:
...
Además se apoyará en un topic compacto sin límite de retención, denominado \newline \textit{TBL.VOCABULARY.CALLS}, que tendrá como clave los \textit{tokens} existentes en el vocabulario y,  como valor, un identificador para cada uno de ellos.
...
El motivo principal para usar \textit{API REST} en este microservicio se debe al uso de la tecnología  \textit{Tensorflow Serving} comentada en el capítulo anterior y que nos permite con mucha facilidad desplegar nuevos modelos y algoritmos, manteniendo la misma estructura en el servidor y la misma \textit{API}. Además su integración es automática con los modelos generados con \textit{TensorFlow} (con o sin \textit{Keras}). Esta decisión permite a los \textit{data-scientist} desplegar nuevas versiones de sus modelos ya sea por degradación o por mejora, mientras el resto del flujo permanece inalterable. 
...
El \textit{topic} de entrada del microservicio \textit{Sequencer} será el topic  \textit{SEQUENCES.CALLS} descrito en el servicio \textit{Sequencer}. 
